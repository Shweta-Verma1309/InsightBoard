...
exports.createPost = async (req, res, next) => {
  try {
    const post = await Post.create({ ...req.body, board: req.params.boardId, createdBy: req.user._id });
    res.status(201).json(post);
  } catch (err) { next(err); }
};

exports.getPostsByBoard = async (req, res, next) => {
  try {
...
exports.votePost = async (req, res, next) => {
  try {
    ...
    if (type === 'up') {
      ...
      post.upvotes.includes(userId) ? post.upvotes.pull(userId) : post.upvotes.push(userId);
    } else {
      post.upvotes.pull(userId);
      post.downvotes.includes(userId) ? post.downvotes.pull(userId) : post.downvotes.push(userId);
    }
    await post.save();
